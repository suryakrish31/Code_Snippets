...
def samp_comp(file1, file2):
    with open(file1, 'rb') as f1:
        ...
        f1_data = [struct.unpack('<I', f1_buf[x:x+4])[0] for x in range(0,len(f1_buf), 4)]
       # f1_data = f1_data[1::2]
    ...
    with open(file2, 'rb') as f2:
        ...
        f2_data = [struct.unpack('<I', f2_buf[x:x + 4])[0] for x in range(0, len(f1_buf), 4)]
       #f2_data = f2_data[::2]
...
if __name__ == '__main__':
   ...
   # file1 = file_path + "Mode1_Q_sample_50MHZ.bin"
    # file1 = file_path + "rrc_verilog_bpsk_out.bin"
    file2 = file_path + "rrc_qpsk_200MHZ.bin"
